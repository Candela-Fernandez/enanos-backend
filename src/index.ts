...
app.post("/api/personas", async (req, res) => {
  ...
  // La edad debe ser un numero entero mayor a 0.
...
app.patch("/api/personas/:id", async (req, res) => {
  ...
  if (arrivalDate !== undefined) {
    if (arrivalDate === null) {
      ...
      if (!parsedArrivalDate) {
        return res
          .status(400)
          .json({ error: "La fecha de llegada no es valida" });
      }
  ...
  if (isWorking !== undefined) {
    data.isWorking = Boolean(isWorking);
  }
...
// Elimina de la base de datos la persona indicada por su id.
app.delete("/api/personas/:id", async (req, res) => {
  ...
  try {
    ...
    // 204 indica que se elimino correctamente y no devuelve contenido.
    res.status(204).end();
  } catch (error: any) {
    // P2025 significa que Prisma no encontro el registro.
    if (error.code === "P2025") {
